...
async function loadBusinessPages() {
  ...
  try {
    // 1. Fetch index.json
    const indexRes = await fetch('https://aedtpworldawfl.github.io/awflmeta/business/pages/index.json');
    ...
    if (!pages.length) {
      container.innerHTML = '<p style="padding:40px;color:#666;font-family:monospace;">No business pages found.</p>';
      return;
    }

    // 2. Loop through each business entry
    for (const item of pages) {
      try {
        const htmlRes = await fetch(`https://aedtpworldawfl.github.io/awflmeta/business/${item.file}`);
        ...
        card.innerHTML = `
          <div class="card-header">
            <h2>${item.name}</h2>
            <span class="card-tag">BUSINESS PAGE</span>
          </div>
    ...
    status.innerHTML = `<span style="color:#ff4d00;">âš  Failed to load business index: ${err.message}</span>`;
    console.error(err);
  }
}
